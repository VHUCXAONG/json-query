...
State.prototype = {
  ...
  setCurrent: function(key, value, options=null){
    console.log(options)
    // console.log(value)
    if (this.currentItem || this.currentKey || this.currentParents.length>0){
      ...
      if (!options){
        this.currentGroup.push({value: this.currentItem})
      }
      else if (options.deepAccess) {
        this.currentGroup.push({deepAccess: true, value: this.currentItem})
      }
      else if (options.index){
        var v = []
        for (var i in options.index){
          v.push(this.currentGroup[this.currentGroup.length - 1].value[i])
        }
        this.currentGroup.push({index: options.index, value: v})
      }
    }
